...
public class JugarController {
    ...
    public String manejarJuego(int idPartida, String accion, HttpSession session) {
        ...
        if (!Jugar.VerificarFlag(idPartida, 1)) {
            if (accion.equals("continuar")) {
                return Jugar.Inicio1();
            }
            if (esAccionValida(accion, 1, 6)) {
                mensaje = Jugar.Inicio2(idPartida, accion);
                ActualizarAtributosPersonaje(session,idPartida);
                return mensaje;
            }
            return "no es valido";
        }
        ...
        if (!Jugar.VerificarFlag(idPartida, 2)) {
            if (esAccionValida(accion, 1, 3)) {
                return Jugar.Inicio3(idPartida, accion);
            }
            return "no es valido";
        }

        return "Estamos en eso jeje 67";
    }
