...
public class cuentas {
	...
	public static boolean VerificarCorreoUso(String correo) {
		String queryRegistro1 = "SELECT correo FROM usuarios";
	...
	public static boolean RegistrarCuenta(String correo,String contra,String nombre) {
		String queryRegistro = "INSERT INTO usuarios (correo,contra,nombre) VALUES (?,?,?)";
		ConexionContra cc = new ConexionContra();
	    Connection conn = cc.conectar();
	...
	public static boolean IDcontra(int id, String contra) {
	    String sql = "SELECT contra FROM usuarios WHERE id_usuario = ?";
	    ConexionContra cc = new ConexionContra();
	    Connection conn = cc.conectar();
	    ...
	    try (PreparedStatement stmt = conn.prepareStatement(sql)) {
	        ...
	        if (rs.next()) {
	            ...
	            return contraBD.equals(contra);
	        }
	    }catch (SQLException e) {
			System.err.println("error al encontrar comprobar contraseña");
	        e.printStackTrace();
		}
	    return false;
	}
